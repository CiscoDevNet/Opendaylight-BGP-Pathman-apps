 ...
 */
(function (nx, global) {
    nx.define('pathman.viewmodel.CreatePath', nx.data.ObservableObject, {
        statics: {
            SelectPathMode: {
                ENDPOINT: 'end-point',
                MANUAL: 'manual'
        ...
        methods: {
            init: function (inParent, view) {
                ...
                view.model(this);
                this.endpointSelector(new pathman.viewmodel.EndpointSelector(this,endpointSelectorView));
                this._metricSelector = endpointSelectorView.view('metricSelector').model();
                this.footer(new pathman.viewmodel.CreatePathFooter(this, view.view('footer')));
            },
            _reset: function () {
            ...
            loadCreatablePaths: function () {
                ...
                nx.backend.path(src, dest, metric, function (paths, costs) {
                    ...
                    for (var i = 0; i < paths.length; i++) {
                        var path = paths[i];
                        var cost = costs[i];
                        self.entries().add(new pathman.viewmodel.CostPath(self, i, cost, path));
                    }
                });
                this.showPath(true);
            },
...
//                    console.log(data);
            ...
            _manualClicked: function (sender, event) {
                ...
                var costPath = new pathman.viewmodel.CostPath(this, 0, 0, []);
